...
#'
#' FORKED BETA VERSION; FOT TEST PURPOSES ONLY
#'
...
#' @md
#' @export
analyse_SAR.CWOSL_beta <- function(
  object,
  signal.integral.min,
  signal.integral.max,
...
#  })
  ...
  if("main"%in% names(list(...))){
   ...
   temp <- lapply(1:length(object), function(x){
    analyse_SAR.CWOSL_beta(object[[x]],
...
# Protocol Integrity Checks --------------------------------------------------
  ...
  if (length(error.list) == 0) {
    ...
    if (length(OSL.Curves.ID) < 2) {
      stop("[analyse_SAR.CWOSL()] Not at least two records of type '", CWcurve.type,"' available" )
    }
