...
params:
  ...
  image_path: !r c(NULL)
---
Average curve analysis of the data set **`r object_name`** performed by the function `RLum.OSL_global_fitting()` from the **R** package `OSLdecomposition`  `r packageVersion("OSLdecomposition")` executed at `r Sys.time()`.
...
## Fitting results {.tabset .tabset-pills}

This report displays the results of [Step 1](#basics) in the component separation analysis workflow described in [Mittelstraß *et al.* (2019)](#disclaimer). The goal of Step 1 is to evaluate the number of components $K$ and determine their decay constants $\lambda_i$. To achieve this, we assume constant CW-OSL decay rates over all measurements of the data set and calculate one [global average curve](#M1). On this, we perform a series of [multi-exponential decay model fittings](#M2) with an increasing number of components *K*:

### Global average
```{r step1_draw_global_curve, fig.width=7, fig.asp=.5}
...
cat(paste0("<br><i><b>Figure 1.1:</b> <u>Blue line</u>: Global average curve built from the arithmetic mean of all curves. <br><u>Grey points</u>: Data points of all measurements. <u>Red</u>: First CW-OSL curve of the first aliquot in the data set.</i><br><br>"))
```
...
Selecting the best fitting model is not trivial. According to the literature, CW-OSL measurements of quartz are sufficiently described by three components ($K = 3$) in the most cases [Bailey et al. 1997](#ref). To prove this default hypothesis or to find out if a model with another number of components suites the data set better, the following analysis tools shall help to select the correct number of components $K$:

1. Comparision of the photo-ionisation cross-section with literature values
2. Evaluation of the fitting quality by an F-test
3. (ToDo:) Evaluation of the decomposition quality by signal bin overdispersion

### Photo-ionisation cross-section


Under the assumption the provided optical stimulation parameters ($\lambda_{stim}=$ `r fit_data$parameters$stimulation.wavelength` $\mathrm{nm}, \Phi_{stim}=$ `r fit_data$parameters$stimulation.intensity` $\mathrm{\frac{mW}{cm^2}}$) are correct, the found decay rates can be translated into [photo-ionisation crosssection values](#M3) and set into relation with quartz OSL literature values:
...
# <font color=\"black\"> </font>
  ...
  cat("<br><i><b>Figure 2:</b> Comparison of the photo-ionisation cross-sections calculated from the signal component decay rates assuming a stimulation light intensity of", fit_data$parameters$stimulation.intensity, "mW/cm² and a stimulation light wavelength of", fit_data$parameters$stimulation.wavelength,"nm.</i><br><br>")
```

Please be aware that stimulation intensity value are prone to false estimations. Such false estimations lead to systematic shifts in the photo-ionisation cross-sections compared to the literature values. To manually correct for these shifts, apply the following rules when setting `RLum.OSL_global_fitting(stimulation_intensity = ?)`:
...
[Bluszcz & Adamiec (2006)](#ref) suggest the use of a [F-test](#M4). The *F*-value measures the improvement in the fitting quality from an additional component by comparing the residual square sum (*RSS*) of the new model with the previous model. Simulations (see Mittelstrass (2019)) show that a threshold of $F = 150$ is ideal to detect the correct number of components for quartz CW-OSL measurements. Falls the *F*-value below thiw threshold, the model with $(K-1)$-components is selected.
...
cat(paste0("<br><i><b>Table 2:</b> Decay constants and fitting quality parameters in dependence of component number $K$. <br><u>RSS</u>: Residual square sum. <u>$F_K$</u>:  Measure of fitting improvement.</i><br><u>Green stripe</u>: Model selected by the F-test<br>"))
...
In our experience however, the F-test performed on real data sets tend to select the $K=4$ model, while the $K=3$ model leads to more plausible results.
...
The underlying algorithm was proposed and described by [Bluszcz & Adamiec (2006)](#ref).

*... EXPAND TEXT HERE ...*


### Photo-ionisation cross-sections {#M3}
...
Mittelstraß, D., 2019. Decomposition of weak optically stimulated luminescence signals and its application in retrospective dosimetry at quartz, Master thesis, TU Dresden, Dresden. <br> https://iktp.tu-dresden.de/IKTP/forschung/Pub.php
