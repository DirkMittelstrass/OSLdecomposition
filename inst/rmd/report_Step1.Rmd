...
Constant signal component decay rates $\lambda$ over all measurements were assumed and one [global average CW-OSL curve](#Overview) was calculated. On this, a series of [multi-exponential decay model fittings](#Overview) with an increasing number of components $K$ was performed. The resulting CW-OSL models can be compared by their photoionisation cross sections $\sigma$ and their fitting quality to select the best suiting set of signal components.
...
if (!is.null(image_format)) image_file <- paste0(image_path, "step1_fig1_1_global_average.", image_format)
...
dynamic_code <- c(
    "#### K = {{i}}\n",
    "```{r tab_{{i}}}\n",
    "if (!is.null(image_format)) image_file <- paste0(image_path, \"step1_fig1_{{i+1}}_K={{i}}_fitting.\", image_format)\n",
...
Details on step 1.1 and step 1.2 can be found in [Mittelstraß (2019)](#ref) and the *R* function documentation, accessible through the commands `?sum_OSLcurves` and `?fit_OSLcurve`. Step 1.2 is based on the ideas of [Bluszcz & Adamiec (2006)](#ref). 
...
[Bluszcz & Adamiec (2006)](#ref) proposed the use of an *F*-test to compare CW-OSL models. The test criterion $F_K$ measures the improvement in the fitting quality from an additional component by comparing the residual square sum (*RSS*) of the $K-1$ model with the new $K$ model:
