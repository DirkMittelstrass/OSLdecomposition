...
for (j in 1:length(record)) {
  ...
  colnames(aliquot_table) <- c("data index", "dose (s)")
  ...
  # Build x-Axis
  x_labes <- NULL
  for (i in 0:((N-1)/2)) x_labes <- c(x_labes, paste0("L", i), paste0("T", i))
...
dynamic_code <- c(
  ...
  # Start dynamic code section with an R chunk
  "```{r tab_{{j}}_result_barplot, fig.asp=0.45}\n",
  ...
  "of measurement sequence `r {{j}}`. Just component which are bleached to at least 95 % during",
...
### Result summary {.tabset .tabset-pills}

The following tables summarize all in Step 2 calculated values for each `RLum.Analysis` object (= aliquot). The column *record index* refers to the index needed to plot a single measurement by calling:

```{r single_plot_explanation, eval=FALSE, include=TRUE}
plot_OSLcurve(data_set[[aliquot_index]]@records[[record_index]])
```
...
# Print the full intensity table
    "<i><b>Table 1.`r {{j}}`:</b> Result summary for aliquot #{{j}}. </i><br>",
