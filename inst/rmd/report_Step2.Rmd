...
`r record_type` component separation report for data set **`r object_name`** analysed by function `RLum.OSL_decomposition()` at `r Sys.time()`; *R* package [**OSLdecomposition**](http://luminescence.de/) version  `r packageVersion("OSLdecomposition")` by [Mittelstraß *et al.* (2019)](#disclaimer)
...
dynamic_code <- c(
    ...
    "if (!is.null(image_format)) {
        image_file <- paste0(image_path, \"step2_fig2_{{j}}_First_records_aliquot_{{j}}.\", image_format)\n
    ...
    "<u>Middle</u>: Residual curves between decomposition model and measurement<br>",
    "<u>Lower right</u>: Estimated type of quartz OSL component (colored), decay rates $\\\\lambda$, signal intensity $n$ and share at first channel $I_0$.</i><br><br>\n")
...
The behaviour of the signal component intensities in the course of measurement sequences contains information about irradiation and bleaching response and thermal- or photo-transfer effects of the associated defect states. Fig. 2 gives an overview of the progression of the component intensities in the course of the measurement sequences. Just components which were bleached to at least 50 % during the analysed time window are displayed, to prevent scaling issues and data misinterpretation due to background correction issues.
...
dynamic_code <- c(
  ...
  # Save image
  "if (!is.null(image_format)) {
      image_file <- paste0(image_path, \"step2_fig1_{{j}}_Intensities_aliquot_{{j}}.\", image_format)\n
  ...
  "at the end of the curve are displayed.",
  "   </i><br><br>\n")
...
Step 2.2 is performed by `decompose_OSLcurve()` for each measurement saperately and can be summarized as following:

1. Set value of each signal bin equal to the integrated form of the [basic equation](#Basics) and get $K$ equations with just the component intensities $n_1$ to $n_K$ unknown.
2. Build an equation system, solve it by [Cramer's rule](https://en.wikipedia.org/wiki/Cramer%27s_rule) and get $n_1$ to $n_K$.
3. Refine $n_1$ to $n_K$ by a Levenberg-Marquardt fitting performed by the function `nlsLM` from the package `minpack.lm` [(Elzhov et al. 2016)](#ref). This step is optional and enhances the result reliability in presence of an uncorrected signal background level. But it decreases error estimation accuracy. 
...
Be aware that error values $\sigma$ are account just for statistical noise (e. g. [shot noise](https://en.wikipedia.org/wiki/Shot_noise) and [dark current noise](https://en.wikipedia.org/wiki/Dark_current_(physics))). Errors in the decay rates or not-first-order OSL kinetics may also cause errors in $n$ without increasing $\sigma$ enough to sufficiently account for them.
