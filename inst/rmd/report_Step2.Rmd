...
The data set is divided into `r length(data_set)` `RLum.Analysis' objects. Each RLum.Analysis objects represents one measurement sequence and therefore the measurement of one aliquot or one grain in case of single grain data.
...
for (j in 1:length(data_set)) {
  ...
  for (k in 1:K) {
    ...
    colnames(new_col) <- paste0("$n_", k, " ± \\sigma_", k,
                                "$ (", input_table$name[k] ,")")
