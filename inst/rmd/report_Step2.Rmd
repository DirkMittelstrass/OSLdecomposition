...
A `r K`-component exponential decay model is assumed globally. Therefore, each `r record_type` measurement can be described by:
...
With the following decay rates $\lambda$ determined in Step 1 or given by the user:

```{r step2_decay_rate_list}
for (k in 1:K) {
  cat(paste0("* $\\lambda_", k,"=", round(input_table$lambda[k], digits = 4),"$ $s^{-1}$ ",
             "(", input_table$name[k] ,")\n"))
}
```

The decompositon process calculates the signal component intensities $n$ and their standard deviations $\sigma$. The intensity values $n$ equal the areas under the component curves (including the remaining virtual signal beyond the stimulation end) and have the same unit as the detector signal (usually *counts* or *mA*). Be also aware that the uncertainty values $\sigma$ account just for the signal noise. Errors in the decay rate values or the decay kinetics may lead to intensity value $n$ discrepancies without increasing the uncertainty value $\sigma$ enough to sufficiently account for them.

```{r step2_create_result_tabs} 
# Build result tables

intensity_tables <- list()
for (j in 1:length(data_set)) {
  result_fragment <- dec_data$results[dec_data$results$list.index == j,]
  aliquot_table <- subset(result_fragment, select = c(record.index, IRR_TIME))
  colnames(aliquot_table) <- c("record index", "irradiation time (s)")
  
  for (k in 1:K) {
    intensities <- result_fragment[,colnames(result_fragment) == paste0("n.", k)]
    sigmas <- result_fragment[,colnames(result_fragment) == paste0("n.error.", k)]
...
dynamic_code <- c(
    "### #{{j}}\n",
    "```{r tab_{{j}}_OSL_plots}\n", # Draw first two plots side by side
    ...
    "```\n",
    "<i><b>Figure 1.`r {{j}}`:</b> Signal components and residual curve of first two `r record_type` records.<br>",
    "<b>Left:</b> First record (index = `r record_index[[{{j}}]][1]`),",
    " usually natural or recovered dose signal ($L_n$).<br>",
    "<b>Right:</b> Second record (index = `r record_index[[{{j}}]][2]`),",
    " usually test-dose signal ($T_n$).</i><br><br>",
    "***\n", # Now print the overview table
    ...
    "```{r tab_{{j}}_result_tables}\n",
    "kable(intensity_tables[[{{j}}]], escape = TRUE, align = \"c\")\n",
    "```\n"
  ) 
