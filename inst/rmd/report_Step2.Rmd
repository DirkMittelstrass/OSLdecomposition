...
dynamic_code <- c(
    ...
    "```{r tab_{{j}}_OSL_plots, fig.asp=0.8}\n", 
...
To prevent scaling issues and data misinterpretations, just components which were bleached to at least 95 % during the analysed time window were used.
...
Details on step 2.1 and step 2.2 can be found in [Mittelstraß (2019)](#ref) and the *R* function documentation, accessible through the commands `?optimise_OSLintervals` and `?decompose_OSLcurve`. 

Step 2.1 used the differential evolution approach (see [Price et al. 2006](#ref)) which was performed by the function `DEoptim` from the package `DEoptim` [(Ardia et al. 2020)](#ref).  .... The Levenberg-Marquardt fitting was performed by the function `nlsLM` from the package `minpack.lm` [(Elzhov et al. 2016)](#ref).
...
Be aware that the uncertainty values $\sigma$ account just for the signal noise. Errors in the decay rate values or the decay kinetics may lead to intensity value $n$ discrepancies without increasing the uncertainty value $\sigma$ enough to sufficiently account for them.
