---
title: "Step 1 perfomance tests (v2)"
date: 2020-08-12
...
Lambda values are calculated from cross sections found by Durcan & Duller an assuming an stimulation intensity of 30 mW/cm² (typical for a Risö OSL/TL DA-15)

Equation: 
lambda = cross.section * flux
...
for (j in 1:N) {
 ...
 if (attr(DE_min, "class") == "try-error") {
   ...
   cat("Fitting failed at scenario:", j, "!\n")
 }else{
  C.list[[j]]$time <- C.fit$parameters$computing.time.s
  C.list[[j]]$info <- C.fit$info.text
  C.list[[j]]$output <- C.fit$F.test }
 
 counter <- counter - 1
 if (counter == 0) {
   counter <- 300
   cat(round(100*j/N), "% done after",difftime(Sys.time(), time.start, units = "h"), "h\n")
 }
}


#cat("(time needed:", difftime(Sys.time(), time.start), "\n")
difftime(Sys.time(), time.start)


```

time comsumption: 
40 min for 72 scenarios on the notebook (K.max = 3)
1 h 30 min for 72 scenarios on the notebook (K.max = 5)
7 h for 324 scenarios on the notebook (K.max = 5)
...
for (j in 1:N) {
  ...
  if (F.y >= N.comp) {
    ...
    if (F.y > N.comp)  {
      F.next <- F.test$F_value[N.comp + 1]
    } else {
      F.next <- NA
    }
...
cat("Cases with a sufficient number of components found: ", nrow(R.table), "of",N , "\n")
...
R.table <- R.table[!is.na(R.table$F),]
cat("Cases with accurate component decay values: ", nrow(R.table), "of",N,"\n")
```
...
C.table <- cor(R.table[R.table$Algorithm==2,], 
               use = "complete.obs", method = "kendall")
write.csv(C.table, paste0(path,"/correlation_table_DE.csv"))



C.table <- cor(R.table[R.table$Algorithm==3,], 
               use = "complete.obs", method = "kendall")
write.csv(C.table, paste0(path,"/correlation_table_DE+LM.csv"))
