...
creating new data.frame and vector objects. So my idea
...
time.start <- Sys.time()
fit_OSLcurve_dev(test_curve, stimulation.wavelength = 535)
cat("(time needed:", round(as.numeric(difftime(Sys.time(), time.start, units = "s")), digits = 2),"s)\n\n")

time.start <- Sys.time()
fit_OSLcurve(test_curve, stimulation.wavelength = 535)
cat("(time needed:", round(as.numeric(difftime(Sys.time(), time.start, units = "s")), digits = 2),"s)\n\n")
