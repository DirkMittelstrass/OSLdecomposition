...
#'
plot_OSLcurve2 <- function(curve = NULL,
                          components = NULL,
...
'    scale_colour_manual(values = graph.colors, labels = graph.labels, guide = guide_legend(title = NULL)) +
    ...
    p.res <- ggplot(curve, aes(x=time, y=residual)) +
      ...
      scale_y_continuous(limits = c(- res.max, res.max), labels = scientific) +
      ylab("residual [cts]") + xlab("time [s]")  +
    ...
    if (display == "detailed" || (display == "compare_lin") ||
      ...
      if (display == "lin-log") {
        ...
        p.linlog <- ggplot(data, aes(x=time, y=signal, colour=graph, size=graph, shape=graph, linetype=graph)) +
          geom_point(na.rm = TRUE) + geom_line(na.rm = TRUE) +
          scale_y_log10(limits = log_limits,
      ...
      if ("n.error" %in% colnames(components)) {
        ...
        p.colnames <- c(p.colnames,
                        expression(italic(sigma[italic(k)])))
      }
