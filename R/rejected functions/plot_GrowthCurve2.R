...
plot_GrowthCurve2 <- function(
  ...
  # scale LxTx errors down
  sample[,3] <- sample[,3] * scaling
  ...
  if (fit.method=="EXP" | fit.method=="EXP OR LIN" | fit.method=="LIN"){
    ...
    if(fit.method!="LIN"){
      ...
      if (inherits(fit, "try-error") & inherits(fit.initial, "try-error")){
        ...
        ##write D01.ERROR
        D01.ERROR <- sd(var.b, na.rm = TRUE) / scaling
  ...
  #EXP+EXP#
    ...
    if (class(fit)!="try-error") {
      ...
      ##write D01.ERROR
      D01.ERROR <- sd(var.b1, na.rm = TRUE) / scaling
      D02.ERROR <- sd(var.b2, na.rm = TRUE) / scaling
  ...
  De.Error <- sd(na.exclude(x.natural)) / scaling
