...
analyse_SAR.CWOSL.deconvolved <- function(
  ...
  X <- c(1:component.number)

  ###  creating data structure
  results <- list(NULL)

  for (x in X) {
    results[[x]] <- set_RLum(class = "RLum.Results",
                             name = components$name[x],
...
'          superposition_curve <- calc_superposition.curve(object,
          ...
          superposition_curve <- mean_curve

          f_fast <- components$lambda[1]
          f_medium <- NA
          try(f_medium <- components$lambda[2])
          f_slow <- NA
          try(f_slow <- components$lambda[3])
